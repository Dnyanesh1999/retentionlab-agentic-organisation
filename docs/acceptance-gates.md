...
- [ ] Submission document contains every required section — assembled by student
- [ ] Student writes the reflection independently — locked; must be non-AI

A local release-readiness slice adds fail-closed release tooling and evidence without any
deployment. It proves two Gate 10 items locally: (1) the complete-codebase ZIP is built from
`git archive HEAD` (tracked files only, byte-stable per commit) and passes a deny-list plus a
shared secret scan before it is written, with a sha256 manifest; and (2) automated
