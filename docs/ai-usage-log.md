...
- Student responsibility: review the strengthened prompt and the output-quality gate, and run a fresh live Manager stage to obtain and approve an accepted `manager.v1.1.0` decision before Gate 8 is accepted for Gate 9 orchestration.
- Verification: the rejected `manager.v1.0.0` live decision is recorded; `manager.v1.1.0` is now required and no accepted `manager.v1.1.0` live run is claimed yet. 16 Manager tests (including the new adversarial truncated-prose, unexpected-glyph and successful bounded-correction cases) plus the full 147-test suite, the agent TypeScript check, ESLint and the production build passed. Full evidence is in `docs/qa-gate-8-manager.md`.
