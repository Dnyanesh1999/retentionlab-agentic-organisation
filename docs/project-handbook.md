...
GitHub Pages hosts the public React application. Supabase hosts the live evidence, clarification, run gateway and all five protected workers. Every worker claims a service-only lease, reads only sealed private predecessors, stores one hash-addressed private artefact and emits bounded public events. The Control Room now runs Researcher through Manager live and stops at mandatory human approval. The local orchestrator remains a separate reproducible, hash-chained assessment implementation.
...
- **`#/control-room` — Control Room.** Select a fictional live account, open the launch sheet and start a governed run. The page polls only recorded backend events as Researcher, Designer, Maker, Communicator and Manager start and seal. A failed stage remains visible and can be retried from the last sealed checkpoint. Success stops at “awaiting human approval”; no external customer action is permitted.
...
4. **Live proof — 60 seconds.** Open `#/control-room`, launch a governed synthetic account and show the five-stage trace moving only when recorded backend events change. Explain checkpoint retry, the account-specific consent channel, and the final human boundary. Then open the live Signal Garden from Experience.
...
Yes. The Control Room invokes all five through the protected Supabase Edge runtime and displays only recorded public events. Full artefacts remain private. The Organisation screen separately renders the committed assessed transcript so the presentation remains reproducible.
...
- The public run intake is intentionally limited to synthetic accounts and is not operator-authenticated or rate-limited for a real organisation yet.
